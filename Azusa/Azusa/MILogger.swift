...
struct MILogger {
    ...
    static func log(_ object : Any, level : MILoggerLevel) {
    ...
    static func saveTo(file : String) {
        MILogger.log("MILogger: Saving all log output to \"\(file)\"", level: .regular);
        ...
        // If there is an error with saving the file...
            ...
            // Print the error
            MILogger.log("MILogger: Error saving log file to \"\(file)\", \(error.localizedDescription)", level: .regular);
        }
    }
}
