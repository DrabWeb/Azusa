...
class MISong: NSObject {
    ...
    override var debugDescription : String {
        return "\(self): \(self.displayTitle) by \(self.displayArtist)(\(AZMusicUtilities.secondsToDisplayTime(self.length))), in \(self.displayAlbum)";
    }
    ...
    static func from(songList : String, log : Bool = true) -> [MISong] {
        if(log) {
            AZLogger.log("MISong: Init song list with string: \"\(songList)\"", level: .full);
        }
    ...
    init(string : String, log : Bool = true) {
        ...
        if(log) {
            AZLogger.log("MISong: Init with string: \"\(string)\"", level: .full);
        }
